...
internal static class Sim09ResidentCoreSmoke
{
    ...
    private static void VerifyGoap()
    {
        ...
        var fallbackRegistry = new ResidentFallbackBehaviorRegistryV1([
            new StableToken("resident.fallback.safe")
        ]);

        var result = ResidentGoapPlannerV1.Plan(start, goal, edges, static _ => 0, fallbackRegistry);
        var reverse = ResidentGoapPlannerV1.Plan(start, goal, edges.Reverse(), static _ => 0, fallbackRegistry);
        ...
        var noPlan = ResidentGoapPlannerV1.Plan(
            ...
            static _ => 0,
            fallbackRegistry);
        Require(noPlan.Status == ResidentGoapStatusV1.NoPlan &&
                noPlan.FallbackAction == new StableToken("resident.fallback.safe"),
            "domain.resident.goap.no-plan: no-plan path must use deterministic fallback registry.");

        var budgetExceeded = ResidentGoapPlannerV1.Plan(
            start,
            goal,
            [
                new ResidentGoapEdgeV1(start, viaA, new StableToken("resident.action.a"), 1),
            ...
            fallbackRegistry,
            expansionBudget: 1);
        Require(budgetExceeded.Status == ResidentGoapStatusV1.BudgetExceeded &&
                budgetExceeded.ExpandedNodes == 1 &&
                budgetExceeded.FallbackAction == new StableToken("resident.fallback.safe"),
            "domain.resident.goap.bound: bounded planner must return stable budget-exceeded fallback.");
    }
