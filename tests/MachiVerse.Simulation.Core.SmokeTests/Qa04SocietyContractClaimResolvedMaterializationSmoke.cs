...
internal static class Qa04SocietyContractClaimResolvedMaterializationSmoke
{
    ...
    internal static void Run()
    {
        ...
        var blockerCodes = Qa04SocietyContractClaimDependencyContractV1.Blockers
        ...
        Require(blockerCodes.SetEquals(new[]
            ...
            "Resolved ContractClaim mechanics must not release the two canonical authority blockers.");
        ...
        var records = Qa04SocietyContractClaimResolvedMaterializerV1.MaterializeResolved(
                localOrdinal => Authority(localOrdinal, resolver),
                resolver)
            .ToArray();
        ...
        for (var index = 0; index < records.Length; index++)
        {
            ...
            Require(record.Payload.ContractKind == expectedAuthority.ContractKind &&
                    record.Payload.PartyRefs.SequenceEqual(expectedAuthority.PartyRefs) &&
            ...
            Require(record.Payload.TermsDigest.SequenceEqual(
                    Qa04SocietyContractClaimDependencyContractV1.TermsDigest(record.RecordId)),
                "Resolved ContractClaim terms_digest must come from the canonical genesis source.");
        }

        var partyDistribution = records
            .GroupBy(static record => record.Payload.PartyRefs.Single())
        ...
        var partition = new DomainPartitionStateV1<SocietyContractClaimPayloadV1>(
            StandardDomainPartitionRegistry.Get(SocietyContractClaimPayloadV1.PartitionId),
            records);
        Require(partition.ItemCount == Qa04SocietyContractClaimResolvedMaterializerV1.CanonicalCount,
            "Resolved ContractClaim partition must retain all 60,000 records.");

        var missingKindRejected = false;
        try
        {
            _ = Qa04SocietyContractClaimResolvedMaterializerV1.CreateResolved(
                0,
                new Qa04SocietyContractClaimResolvedAuthorityV1(default, new[] { resolver.RefFor(0) }),
                resolver,
                out _);
        }
        catch (InvalidDataException ex) when (ex.Message == "qa04.society.contract-kind-authority-required")
        {
            missingKindRejected = true;
        }
        Require(missingKindRejected,
            "Resolved ContractClaim materialization must fail closed when contract_kind authority is absent.");
        ...
        try
        {
            _ = Qa04SocietyContractClaimResolvedMaterializerV1.CreateResolved(
                0,
                new Qa04SocietyContractClaimResolvedAuthorityV1(FixtureKindA, Array.Empty<PartitionRecordRefV1>()),
                resolver,
                out _);
        }
        catch (InvalidDataException ex) when (ex.Message == "qa04.society.contract-party-authority-required")
        {
            missingPartyRejected = true;
        }
        Require(missingPartyRejected,
            "Resolved ContractClaim materialization must fail closed when party authority is absent.");
        ...
        try
        {
            ...
            ex.Message == "domain.payload.reference-validation:society.contract_claim:party_refs")
        {
            unresolvedPartyRejected = true;
        }
        Require(unresolvedPartyRejected,
            "Resolved ContractClaim materialization must fail closed when actual party authority is not resolvable.");
    }

    private static Qa04SocietyContractClaimResolvedAuthorityV1 Authority(
        ulong localOrdinal,
        CanonicalPolityFixtureReferenceResolver resolver)
        => new(
            (localOrdinal & 1UL) == 0 ? FixtureKindA : FixtureKindB,
            new[] { resolver.RefFor(localOrdinal % 1_000UL) });
    ...
    private sealed class CanonicalPolityFixtureReferenceResolver : IDomainRecordSchemaResolverV1
    {
        ...
        public CanonicalPolityFixtureReferenceResolver()
        {
            ...
            _refs = Qa04GovernancePolityMaterializerV1.MaterializeCanonical()
                .Select(static record => new PartitionRecordRefV1(GovernancePolityPayloadV1.PartitionId, record.RecordId))
                .ToArray();
            _records = _refs.ToDictionary(static reference => reference, _ => schema);
        }

        public PartitionRecordRefV1 RefFor(ulong localOrdinal)
            => _refs[checked((int)localOrdinal)];
