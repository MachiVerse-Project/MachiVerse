...
public sealed record ParticipationBindResolutionV1(IReadOnlyList<ParticipationBindingStateV1> Accepted, IReadOnlyList<OpaqueId128> RejectedBindingIds);

public static class ParticipationBindResolverV1
{
    public static ParticipationBindResolutionV1 Resolve(IEnumerable<ParticipationBindingStateV1> existingBindings, IEnumerable<ParticipationBindRequestV1> requests)
    {
        ...
        var activeDivers = existing.Where(static binding => binding.IsActive).Select(static binding => binding.DiverRef).ToHashSet();
        var activeResidents = existing.Where(static binding => binding.IsActive).Select(static binding => binding.ResidentId).ToHashSet();
        ...
        {
            if (seenBindingIds.Contains(request.BindingId) || activeDivers.Contains(request.DiverRef) || activeResidents.Contains(request.ResidentId))
            {
            ...
            var binding = new ParticipationBindingStateV1(request.BindingId, request.DiverRef, request.ResidentId, ParticipationBindingStatusV1.Active, request.EffectiveStep, null, request.BindingGeneration);
            binding.Validate();
...
public enum ParticipationControlAvailabilityV1 : byte { Available = 1, Unavailable = 2 }
public enum ResidentControlModeV1 : byte { Autonomous = 1, DiverControlAvailable = 2, DiverAbsentPolicy = 3, BoundResidentDeceased = 4 }

public sealed record ParticipationControlContextV1(OpaqueId128 ResidentId, OpaqueId128? BindingId, ResidentControlModeV1 Mode, uint? AbsencePolicyGeneration, ulong BasisStep);

public static class ParticipationControlContextFactoryV1
{
    public static ParticipationControlContextV1 ForBinding(ParticipationBindingStateV1 binding, ParticipationControlAvailabilityV1 availability, uint? absencePolicyGeneration, ulong basisStep)
    {
        binding.Validate();
        if (!Enum.IsDefined(availability)) throw new InvalidDataException("participation.control-availability-invalid");
        var mode = binding.Status switch
        {
            ParticipationBindingStatusV1.ResidentDeceased => ResidentControlModeV1.BoundResidentDeceased,
            ParticipationBindingStatusV1.Active when availability == ParticipationControlAvailabilityV1.Available => ResidentControlModeV1.DiverControlAvailable,
            ParticipationBindingStatusV1.Active => ResidentControlModeV1.DiverAbsentPolicy,
            _ => ResidentControlModeV1.Autonomous,
        };
        return new ParticipationControlContextV1(binding.ResidentId, mode == ResidentControlModeV1.Autonomous ? null : binding.BindingId, mode, mode == ResidentControlModeV1.DiverAbsentPolicy ? absencePolicyGeneration : null, basisStep);
    }
}

public sealed record ParticipationAbsencePolicyV1(OpaqueId128 DiverRef, OpaqueId128? BindingId, uint PolicyGeneration, IReadOnlyList<StableToken> PriorityRules, ulong EffectiveFromStep)
{
    public void Validate()
    {
    ...
    public ParticipationAbsencePolicyV1 Revise(uint expectedGeneration, IEnumerable<StableToken> priorityRules, ulong effectiveFromStep)
    {
        Validate();
        if (expectedGeneration != PolicyGeneration) throw new InvalidDataException("participation.absence-policy-stale-generation");
        ArgumentNullException.ThrowIfNull(priorityRules);
        var rules = priorityRules.ToArray();
        var next = this with { PolicyGeneration = checked(PolicyGeneration + 1), PriorityRules = Array.AsReadOnly(rules), EffectiveFromStep = effectiveFromStep };
        next.Validate();
        return next;
    }
}

public sealed record ParticipationDetailRequirementV1(OpaqueId128 ResidentId, byte MinimumDetail, ulong EffectiveFromStep, ulong? EffectiveUntilStep)
{
    public void Validate()
    {
    ...
    public bool AppliesAt(ulong step) => step >= EffectiveFromStep && (EffectiveUntilStep is null || step <= EffectiveUntilStep.Value);
}

public static class ParticipationDetailFloorV1
{
    public static byte Resolve(OpaqueId128 residentId, ulong step, IEnumerable<ParticipationDetailRequirementV1> requirements)
    {
        ...
        foreach (var requirement in requirements)
        {
            ...
            if (requirement.ResidentId == residentId && requirement.AppliesAt(step)) floor = Math.Min(floor, requirement.MinimumDetail);
        }
        return floor;
    }
}
