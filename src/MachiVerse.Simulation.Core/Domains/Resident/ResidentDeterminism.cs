...
public readonly record struct ResidentGoapStateV1(string StableStateDigest)
{
    ...
    {
        if (string.IsNullOrWhiteSpace(StableStateDigest))
            throw new InvalidDataException("resident.goap-state-digest-empty");
        return this;
    }
}
...
public static class ResidentGoapPlannerV1
{
    ...
    public static ResidentGoapResultV1 Plan(
        ...
        StableToken fallbackAction,
        int expansionBudget = MaxExpandedNodes)
    {
        start.Validate();
        goal.Validate();
        ArgumentNullException.ThrowIfNull(edges);
        ArgumentNullException.ThrowIfNull(heuristic);
        if (expansionBudget is < 1 or > MaxExpandedNodes)
        ...
        {
            new(start, heuristic(start), 0, new StableToken("goap.start"))
        };
        var expanded = 0;
        ...
        while (open.Count != 0 && expanded < expansionBudget)
        {
            ...
            var current = open[0];
            open.RemoveAt(0);
            if (!bestCost.TryGetValue(current.State, out var currentBest) || currentBest != current.GCost)
                continue;

            expanded++;
            if (current.State == goal)
                return BuildFound(start, goal, current.GCost, expanded, previous);

            if (!adjacency.TryGetValue(current.State, out var outgoing))
                continue;

            foreach (var edge in outgoing)
            {
                var candidateCost = checked(current.GCost + edge.Cost);
                if (bestCost.TryGetValue(edge.To, out var oldCost) && oldCost <= candidateCost)
                    continue;

                bestCost[edge.To] = candidateCost;
                previous[edge.To] = (current.State, edge.ActionToken);
                var fCost = checked(candidateCost + heuristic(edge.To));
                open.Add(new OpenNode(edge.To, fCost, candidateCost, edge.ActionToken));
            }
        }
    ...
    private static ResidentGoapResultV1 BuildFound(
        ...
        ulong totalCost,
        int expanded,
        IReadOnlyDictionary<ResidentGoapStateV1, (ResidentGoapStateV1 Previous, StableToken Action)> previous)
    {
        var actions = new List<StableToken>();
        var cursor = goal;
        while (cursor != start)
        {
            if (!previous.TryGetValue(cursor, out var entry))
                throw new InvalidDataException("resident.goap-path-broken");
    ...
    private sealed record OpenNode(
        ResidentGoapStateV1 State,
        ulong FCost,
        ulong GCost,
        StableToken ActionToken);
