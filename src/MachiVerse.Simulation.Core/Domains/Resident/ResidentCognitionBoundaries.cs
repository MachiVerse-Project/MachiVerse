    ...
    ulong LastUpdatedStep)
{
    public void Validate()
    {
        if (ResidentId.IsZero || SubjectRef.IsZero || EvidenceObservationId.IsZero)
            throw new InvalidDataException("resident.belief-id-zero");
        if (ConfidencePpm > ResidentPpmV1.Max)
            throw new InvalidDataException("resident.belief-confidence-range");
    }
}
...
public sealed class ResidentCognitionProjectionV1
{
    ...
    public IReadOnlyList<ResidentInformationDeliveryV1> PendingDeliveries => Array.AsReadOnly(_deliveries.Values.ToArray());
    ...
    public void ReceiveDelivery(ResidentInformationDeliveryV1 delivery)
    {
        ...
        // Delivery is a transport fact only. Belief authority does not change here.
    }
    ...
    {
        observation.Validate();
        var key = BuildBeliefKey(observation.ResidentId, observation.SubjectRef, observation.Proposition);
    ...
    private static string BuildBeliefKey(OpaqueId128 residentId, OpaqueId128 subjectRef, StableToken proposition)
        => residentId + "/" + subjectRef + "/" + proposition.Value;
}
...
public sealed record ResidentPhysicalActionDecisionV1(
    ...
    ReadOnlyMemory<byte> SemanticPayloadDigest)
{
    public void Validate()
    {
...
public static class ResidentPhysicalIntentFactoryV1
{
        ...
        int semanticPriority)
    {
        ...
        return new MutationIntentCandidateV1(
            ...
            decision.SemanticPayloadDigest.Span);
    }
}
