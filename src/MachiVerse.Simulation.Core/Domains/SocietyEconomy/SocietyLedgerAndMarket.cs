    ...
    IReadOnlyList<LedgerEntryV1> Entries)
{
    public IReadOnlyList<LedgerEntryV1> ValidateAndCanonicalize()
    {
        ...
        foreach (var currencyGroup in Entries.GroupBy(static entry => entry.CurrencyToken.Value, StringComparer.Ordinal))
        {
            ...
            {
                if (entry.EntryKind == LedgerEntryKindV1.Debit)
                    debits += entry.AmountMicrounit;
                else
                    credits += entry.AmountMicrounit;
            }
            if (debits != credits)
...
public static class DeterministicCallAuctionV1
{
    public static MarketClearingResultV1 Clear(IEnumerable<MarketOrderV1> orders, ulong clearingStep)
    {
        ...
        var eligible = orders.Where(order => order.EligibleStep <= clearingStep).ToArray();
        foreach (var order in eligible)
            order.Validate();
        if (eligible.Select(static order => order.OrderId).Distinct().Count() != eligible.Length)
        ...
        var executed = checked(trades.Sum(static trade => trade.Quantity));
