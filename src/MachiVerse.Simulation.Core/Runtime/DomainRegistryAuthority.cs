...
public sealed class DomainRuntimeDescriptorV1
{
    ...
    private static IReadOnlyList<StableToken> FreezeCanonical(IEnumerable<StableToken> values, string error)
    {
        ...
        {
            if (string.CompareOrdinal(material[i - 1].Value, material[i].Value) >= 0)
                throw new InvalidDataException(error);
        }
        return Array.AsReadOnly(material);
    }
}
...
public sealed class DomainRegistryStateV1
{
    ...
    public void RequireAuthorizedEmission(MutationIntentCandidateV1 intent)
    {
        ...
        var source = Get(intent.SourceDomain);
        if (!source.EmittedIntentKinds.Contains(intent.MutationKind))
            throw new InvalidDataException("domain-runtime.unauthorized-emitted-intent");

        var target = Get(intent.TargetDomain);
        if (!target.AcceptedIntentKinds.Contains(intent.MutationKind))
            throw new InvalidDataException("domain-runtime.intent-target-not-accepted");

        if (!StandardDomainRuntimeCapabilityRegistryV1.Generation1IntentCapabilities.Any(value =>
                value.SourceDomain == intent.SourceDomain &&
    ...
    private void ValidateStandardStructure()
    {
        ...
        foreach (var descriptor in Domains)
        {
            ...
            if (descriptor.DomainRank != expected[0].OwnerDomainRank)
                throw new InvalidDataException("domain-registry.domain-rank-mismatch");
            if (descriptor.OwnedPartitions.Count != expected.Length)
                throw new InvalidDataException("domain-registry.missing-partition-owner");
            for (var i = 0; i < expected.Length; i++)
            {
                if (descriptor.OwnedPartitions[i] != expected[i].PartitionId)
...
{
    private static readonly IReadOnlyList<DomainIntentCapabilityV1> Generation1Capabilities = BuildIntentCapabilities();

    private static readonly IReadOnlyDictionary<string, SchemaRefV1> DomainSchemas =
    ...
    private static readonly IReadOnlyDictionary<string, IReadOnlyList<StableToken>> StateReads =
    ...
    private static readonly IReadOnlyList<DomainSameStepDependencyV1> NoGeneration1SameStepDependencies
        = Array.Empty<DomainSameStepDependencyV1>();
    ...
    public static IReadOnlyList<DomainSameStepDependencyV1> Generation1SameStepDependencies => NoGeneration1SameStepDependencies;

    public static SchemaRefV1 GetGeneration1DomainSchema(StableToken domain)
        => DomainSchemas.TryGetValue(domain.Value, out var schema)
            ? schema
            : throw new InvalidDataException("domain-registry.unknown-domain-token");

    public static IReadOnlyList<StableToken> GetGeneration1StateReadDependencies(StableToken domain)
        => StateReads.TryGetValue(domain.Value, out var dependencies)
            ? dependencies
            : throw new InvalidDataException("domain-registry.unknown-domain-token");

    public static IReadOnlyList<StableToken> GetGeneration1SameStepDependencies(StableToken consumerDomain)
    {
        _ = GetGeneration1DomainSchema(consumerDomain);
        // Phase 3 describes same-Step semantic dependencies, but the current Standard
        // generation-1 executor has no production default edge. Do not claim one until
        // the execution plan materializes it as runtime authority.
        return Array.Empty<StableToken>();
    }
    ...
    public static IReadOnlyList<StableToken> GetGeneration1EmittedEventKinds(StableToken domain)
    {
        ...
        // DomainCandidateOutputV1 has no DomainEvent channel in generation 1.
    ...
    {
        _ = GetGeneration1DomainSchema(domain);
        // CrossDomainTransaction invariants are Core authority, not domain emissions.
        return NoGeneration1DomainInvariants;
    }

    private static IReadOnlyList<DomainIntentCapabilityV1> BuildIntentCapabilities()
    {
