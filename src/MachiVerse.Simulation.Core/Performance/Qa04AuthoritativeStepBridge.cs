...
/// path and cross the real SIM-03 SQLite transition COMMIT authority boundary.
/// </summary>
public static class Qa04AuthoritativeStepBridgeV1
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var runtimes = CreateStructuralRuntimes();
        ...
        var transition = CreateTransitionHistory(candidate, genesis.RecordDigest);
        ...
        if (recovery.FinalizedStep != candidate.TargetStep ||
    ...
    private static IReadOnlyCollection<IDomainRuntimeV1> CreateStructuralRuntimes()
    {
        ...
        static ValueTask<IReadOnlyList<PartitionCandidateV1>> ResidentPartition(
            DomainRuntimeContextV1 context,
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var current = context.State.Partitions.Get("resident.identity_lifecycle").Header;
            var changeSetDigest = HashSuite.DomainHash("mv.qa04-structural-resident-step.v1", writer =>
            {
                writer.WriteMapStart(3);
                writer.WriteUnsigned(0); writer.WriteUnsigned(context.FrozenInput.BasisStep);
                writer.WriteUnsigned(1); writer.WriteBytes(current.CanonicalDigest);
                writer.WriteUnsigned(2); writer.WriteUnsigned(current.ItemCount);
            });
