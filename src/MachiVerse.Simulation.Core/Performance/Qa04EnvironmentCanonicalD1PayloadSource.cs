...
/// <summary>
/// Production perf.reference.v1 D1 Environment payload authority. Each aggregate is computed from
/// the exact four canonical D0 payloads selected by the decomposition. No alternate genesis value
/// path exists for D1.
/// </summary>
public sealed class Qa04EnvironmentCanonicalD1PayloadSourceV1 : IQa04EnvironmentD1PayloadSourceV1
{
    ...
    public EnvironmentGeologyPayloadV1 Geology(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentGeologyPayloadV1(
            Scope(binding),
            RequireInvariantTokenList(s.Select(static x => x.MaterialClasses).ToArray(), "geology.material_classes"),
            Union(s.Select(static x => x.StrataRefs).ToArray()),
            Mean(s.Select(static x => x.PorosityPpm).ToArray()),
            Mean(s.Select(static x => x.StabilityPpm).ToArray()),
            Mean(s.Select(static x => x.PermeabilityQ32).ToArray()),
            Union(s.Select(static x => x.FaultRefs).ToArray()),
            Union(s.Select(static x => x.ResourceRefs).ToArray()));
    }
    ...
    public EnvironmentSoilPayloadV1 Soil(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentSoilPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.SoilClass).ToArray()),
            Mean(s.Select(static x => x.DepthMm).ToArray()),
            Mean(s.Select(static x => x.MoisturePpm).ToArray()),
            Mean(s.Select(static x => x.FertilityPpm).ToArray()),
            Sum(s.Select(static x => x.OrganicMassGram).ToArray()),
            Union(s.Select(static x => x.ContaminantRefs).ToArray()));
    }
    ...
    public EnvironmentResourceDepositPayloadV1 ResourceDeposit(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentResourceDepositPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.ResourceKind).ToArray()),
            Sum(s.Select(static x => x.RemainingMassGram).ToArray()),
            Mean(s.Select(static x => x.GradePpm).ToArray()),
            Sum(s.Select(static x => x.RenewalRateGramPerStep).ToArray()),
            Mean(s.Select(static x => x.AccessibilityPpm).ToArray()));
    }
    ...
    public EnvironmentGroundwaterPayloadV1 Groundwater(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentGroundwaterPayloadV1(
            Scope(binding),
            Sum(s.Select(static x => x.WaterVolumeMl).ToArray()),
            Mean(s.Select(static x => x.HydraulicHeadMm).ToArray()),
            Mean(s.Select(static x => x.QualityPpm).ToArray()),
            Mean(s.Select(static x => x.TemperatureMilliKelvin).ToArray()),
            Union(s.Select(static x => x.NeighborRefs).ToArray()));
    }
    ...
    public EnvironmentAtmospherePayloadV1 Atmosphere(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentAtmospherePayloadV1(
            Scope(binding),
            Mean(s.Select(static x => x.PressurePa).ToArray()),
            Mean(s.Select(static x => x.TemperatureMilliKelvin).ToArray()),
            Mean(s.Select(static x => x.HumidityPpm).ToArray()),
            Mean(s.Select(static x => x.WindUmPerSecond).ToArray()),
            Sum(s.Select(static x => x.VaporMassGram).ToArray()),
            Sum(s.Select(static x => x.LiquidMassGram).ToArray()),
            AggregateGas(s));
    }
    ...
    public EnvironmentClimatePayloadV1 Climate(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentClimatePayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.Regime).ToArray()),
            Mean(s.Select(static x => x.TemperatureMeanMilliKelvin).ToArray()),
            Mean(s.Select(static x => x.PrecipitationMeanMl).ToArray()),
            Mean(s.Select(static x => x.WindMeanUmPerSecond).ToArray()),
            Qa04EnvironmentD1AggregationV1.CheckedSum(s.Select(static x => x.SampleCount).ToArray()),
            Qa04EnvironmentD1AggregationV1.MaxPlusOne(s.Select(static x => x.AggregateGeneration).ToArray()));
    }
    ...
    public EnvironmentWeatherPayloadV1 Weather(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentWeatherPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.WeatherClass).ToArray()),
            Sum(s.Select(static x => x.PrecipitationMlPerStep).ToArray()),
            Mean(s.Select(static x => x.CloudPpm).ToArray()),
            Mean(s.Select(static x => x.VisibilityMm).ToArray()),
            Mean(s.Select(static x => x.StormIntensityPpm).ToArray()),
            Qa04EnvironmentD1AggregationV1.MaxPlusOne(s.Select(static x => x.BasisAtmosphereRevision).ToArray()));
    }
    ...
    public EnvironmentSurfaceWaterPayloadV1 SurfaceWater(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentSurfaceWaterPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.WaterBodyClass).ToArray()),
            Sum(s.Select(static x => x.VolumeMl).ToArray()),
            Mean(s.Select(static x => x.SurfaceLevelMm).ToArray()),
            Mean(s.Select(static x => x.FlowUmPerSecond).ToArray()),
            Mean(s.Select(static x => x.TemperatureMilliKelvin).ToArray()),
            Mean(s.Select(static x => x.QualityPpm).ToArray()),
            Union(s.Select(static x => x.DownstreamRefs).ToArray()));
    }
    ...
    public EnvironmentOceanPayloadV1 Ocean(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentOceanPayloadV1(
            ...
            Sum(s.Select(static x => x.WaterVolumeMl).ToArray()),
            Mean(s.Select(static x => x.SurfaceLevelMm).ToArray()),
            Mean(s.Select(static x => x.VelocityUmPerSecond).ToArray()),
            Mean(s.Select(static x => x.TemperatureMilliKelvin).ToArray()),
            Mean(s.Select(static x => x.SalinityPpm).ToArray()),
            Union(s.Select(static x => x.NeighborRefs).ToArray()));
    }
    ...
    public EnvironmentEcosystemPayloadV1 Ecosystem(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentEcosystemPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.SpeciesOrCohort).ToArray()),
            Qa04EnvironmentD1AggregationV1.CheckedSum(s.Select(static x => x.Population).ToArray()),
            Sum(s.Select(static x => x.BiomassGram).ToArray()),
            Mean(s.Select(static x => x.BirthRatePpm).ToArray()),
            Mean(s.Select(static x => x.DeathRatePpm).ToArray()),
            Mean(s.Select(static x => x.MigrationRatePpm).ToArray()),
            Union(s.Select(static x => x.ResourceRefs).ToArray()));
    }
    ...
    public EnvironmentContaminantPayloadV1 Contaminant(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentContaminantPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.ContaminantKind).ToArray()),
            Sum(s.Select(static x => x.StockMassGram).ToArray()),
            Mean(s.Select(static x => x.ConcentrationPpb).ToArray()),
            Union(s.Select(static x => x.SourceRefs).ToArray()),
            Union(s.Select(static x => x.SinkRefs).ToArray()));
    }
    ...
    public EnvironmentHazardPayloadV1 Hazard(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        return new EnvironmentHazardPayloadV1(
            Scope(binding),
            Mode(s.Select(static x => x.HazardKind).ToArray()),
            Mean(s.Select(static x => x.IntensityPpm).ToArray()),
            Qa04EnvironmentD1AggregationV1.MaxStep(s.Select(static x => x.StartedStep).ToArray()),
            Qa04EnvironmentD1AggregationV1.OptionalEndMinimum(s.Select(static x => x.ExpectedEndStep).ToArray()),
            Union(s.Select(static x => x.DriverRefs).ToArray()),
            Union(s.Select(static x => x.AffectedScopeRefs).ToArray()));
    }
    ...
    public EnvironmentLineagePayloadV1 Lineage(Qa04EnvironmentD1BindingV1 binding)
    {
        ...
        var digest = SourceDigest(binding, sources);
        return new EnvironmentLineagePayloadV1(subject, parents, generation, AggregateMaterializationKind, digest);
    }
    ...
    private static T[] Sources<T>(Qa04EnvironmentD1BindingV1 binding, Func<Qa04EnvironmentD0BindingV1, T> factory)
    {
        ...
        return binding.SourceD0GlobalOrdinals
    ...
    private static PartitionRecordRefV1 Scope(Qa04EnvironmentD1BindingV1 binding)
    ...
    private static IReadOnlyList<PartitionRecordRefV1> Union(IReadOnlyList<IReadOnlyList<PartitionRecordRefV1>> values)
        => Qa04EnvironmentD1AggregationV1.CanonicalSetUnion(values);

    private static IReadOnlyList<StableToken> RequireInvariantTokenList(
        IReadOnlyList<IReadOnlyList<StableToken>> values,
        string field)
    {
    ...
    private static IReadOnlyList<KeyValuePair<string, uint>> AggregateGas(IReadOnlyList<EnvironmentAtmospherePayloadV1> sources)
    {
        ...
        {
            var values = sources.Select(source => source.GasPpb[index].Value).ToArray();
            result[index] = new KeyValuePair<string, uint>(keys[index], Mean(values));
        }
    ...
    {
        var ordered = binding.SourceD0GlobalOrdinals
            .Select(Qa04EnvironmentReferenceDecompositionV1.BindD0)
            .Zip(sources, static (source, payload) => new { source.Descriptor.RecordId, Payload = payload })
            .OrderBy(static value => value.RecordId)
            .ToArray();
        return HashSuite.DomainHash("mv.perf-reference-environment-d1-source.v1", writer =>
        {
            writer.WriteArrayStart(ordered.Length);
