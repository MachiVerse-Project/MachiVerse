...
public static partial class GatewayBootstrapEnvelopeCodec
{
    public static byte[] Encode(WireEnvelopeV1 envelope)
    {
        Validate(envelope);
        var bytes = envelope.ToByteArray();
        if (bytes.Length > GatewayEnvelopeCodec.MaxSerializedEnvelopeBytes)
    ...
    private static void Validate(WireEnvelopeV1 envelope)
    {
        ...
        if (!string.Equals(envelope.ProtocolId, GatewayEnvelopeCodec.ProtocolId, StringComparison.Ordinal))
